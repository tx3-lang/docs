...
## Consulting an oracle via a reference input

The `reference` block is not under the `cardano::` namespace — it is a generic tx-body construct — but reading datums off reference inputs is one of the canonical Cardano patterns, so it earns a place here. Where `input` consumes a UTxO, `reference` reads one without spending it.
...
With `datum_is` set, you can read the reference's datum as typed fields (`oracle_data.value`).

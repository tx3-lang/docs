...
Note that running `tx3up` is required to prepare the environment before starting the devnet. For more detailed information on using the development network, please refer to the [Devnet Guide](../authoring/devnet).
...
For detailed information about test configuration format and examples, see the [Testing Guide](../authoring/testing).

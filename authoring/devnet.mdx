...
For information about automated testing with Devnet, see the [Testing Guide](./testing) and the [`trix test` command documentation](../tooling/trix#trix-test).

## Usage

Before running Devnet manually, use [trix init](../tooling/trix#trix-init) to initialize a tx3 project. This will generate a default Devnet configuration in the `trix.toml` file, including default wallets.  
After initializing the project, run [trix devnet](../tooling/trix#trix-devnet) to start the Devnet.
...
- **ref**: The UTXO reference in the format `txid#index` (e.g., `abcd1234...#0`)
- **raw_bytes**: The raw bytes of the UTXO in hex CBOR format
...
# Dep UTXO
[[utxos]]
ref = "001122aabbccddee334455667788990011223344556677889900#0"
raw_bytes = "001122aabbccddee334455667788990011223344556677889900..."
```
...
2. (Optional) Sync devnet with a tx utxo deps:
   ```sh
   trix devnet copy --profile <profile> --utxo-deps <tx-hash>
   ```

   This creates the `devnet.toml` file with the necessary UTXO deps.
...
4. (Optional) Run Devnet in the Background:
   ```sh
   trix devnet -b
   ```
